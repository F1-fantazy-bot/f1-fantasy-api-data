...
async function _fetchLockedTeamSnapshot(entry, teamName) {
  ...
  let oppData;
  try {
    // The `v` parameter on opponent endpoints is the **team_no
    // disambiguator** for users with multiple F1 Fantasy teams in a
    // league. The path-level `teamNo` is empirically ignored when
    // querying opponents (it represents the caller's perspective);
    // only `v` selects which of the opponent's teams to return. Pass
    // `entry.team_no` as `v` so a user with 3 teams (e.g. Dor Segal's
    // dorsegal1/2/3 with team_no 1/2/3) returns 3 distinct rosters
    // instead of collapsing onto whichever team v=1 yields.
    oppData = await f1Api.getOpponentGameDays(entry.user_guid, teamNo, teamNo);
  } catch (err) {
  ...
  //
  // The `v` parameter is the team_no disambiguator (see getOpponentGameDays
  // call above for the same logic).
  ...
  for (const md of candidateMatchdays) {
    ...
    try {
      ...
      console.log(
        `   ⚠️ getOpponentTeam(${md}, v=${teamNo}) failed for ${teamName}: ${err.message}`,
      );
      continue;
    }
